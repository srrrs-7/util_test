...
.PHONY: rds mongo goapi goworker worker-test gql gopher rust rustapi rustworker node deno bun php linux k6 plantuml
rds:
...
gql:
	docker compose up -d gql --build

# example: make gqlgen-init, make gqlgen-generate
gqlgen-%:
	docker compose run --rm gqlgen %
